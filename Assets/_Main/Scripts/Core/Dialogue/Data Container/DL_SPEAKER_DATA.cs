...
namespace DIALOGUE
{
    public class DL_SPEAKER_DATA
    {
        ...
        /// <summary>
        /// Kode dibawah untuk menampilkan siapa yang sedang berbicara
        /// </summary>
        ...
        public DL_SPEAKER_DATA(string rawSpeaker)
        {
            ...
            string pattern = @$"{NAMECAST_ID}|{POSITIONCAST_ID}|{EXPRESSIONCAST_ID.Insert(EXPRESSIONCAST_ID.Length - 1, @"\")}"; //untuk mencari pattern jika pada dialogue terdapat kalimat tersebut dan juga untuk mengambil beberapa perintah nantinya
            MatchCollection matches = Regex.Matches(rawSpeaker, pattern);

            //Membuat data ini untuk menghindari adanya null references ke suatu variable
            castName = "";
            castPosition = Vector2.zero;
            CastExpressions = new List<(int layer, string expression)>();

            //Jika tidak ada matches, maka keseluruhan tulisan adalah speaker name atau karakter
            if(matches.Count == 0)
            {
                name = rawSpeaker;
                return;
            ...
            //Sebaliknya, jika ada maka akan mengisolasi speakerName dari casting data.
