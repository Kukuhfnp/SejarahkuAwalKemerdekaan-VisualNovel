...
namespace DIALOGUE
{
    public class AutoReader : MonoBehaviour
    {
        ...
        private const float MAX_READ_TIME = 30f;
        ...
        public Coroutine co_running = null;
        ...
        private IEnumerator AutoRead()
        {
            ...
            if (!architect. isBuilding && architect. currentText != string. Empty)
                DialogueSystems. instance. OnSystemPrompt_Next();
            ...
            while (conversationManager. isRunning)
            {
                ...
                if (!skip)
                {
                    ...
                    float timeToRead = Mathf.Clamp(((float)architect.tmpro.textInfo.characterCount / DEFAULT_CHARACTER_READ_PER_SECOND), MIN_READ_TIME, MAX_READ_TIME);
