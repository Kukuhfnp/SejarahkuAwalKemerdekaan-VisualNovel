...
namespace COMMANDS
{
    public class CMD_DatabaseExtension_Characters : CMD_DatabaseExtension
    {
        ...
        new public static void Extend(CommandDatabase database)
        {
            ...
            baseCommands.AddCommand("flipright", new Func<string[], IEnumerator>(FlipRight));
            baseCommands.AddCommand("flipleft", new Func<string[], IEnumerator>(FlipLeft));
            ...
            //Add characters specific databases
        ...
        private static IEnumerator FlipRight(string[] data)
        {
            Character character = CharacterManager.instance.GetCharacter(data[0]);
            ...
            parameters.TryGetValue(new string[] { "-s", "-speed"}, out speed, defaultValue: 1);
            parameters.TryGetValue(new string[] { "-i", "-immediate" }, out immediate, defaultValue: false);
            ...
            {
                CommandManager.instance.AddTerminationActionToCurrentProcess(() => {character.FaceRight();});
                yield return character.FaceRight(speed, immediate);
            }
        }
        private static IEnumerator FlipLeft(string[] data)
        {
            Character character = CharacterManager.instance.GetCharacter(data[0]);
            ...
            {
                CommandManager.instance.AddTerminationActionToCurrentProcess(() => {character.FaceLeft();});
