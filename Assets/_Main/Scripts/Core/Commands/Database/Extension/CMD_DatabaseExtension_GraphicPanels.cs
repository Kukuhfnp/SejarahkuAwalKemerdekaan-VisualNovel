...
namespace COMMANDS
{
    public class CMD_DatabaseExtension_GraphicPanels : CMD_DatabaseExtension
    {
        ...
        private static IEnumerator SetLayerMedia(string[] data)
        {
            ...
            //If this is a video, try to get wheter we use audio from video or not
            ...
            if(graphic == null)
            {
                Debug.LogError($"could not find media file called '{mediaName}' in the Resources directories. Please specify the full path within resources and make sure that the file exist!");
                yield break;
            }
            ...
            //Lets try to get the layer to apply to the media to
