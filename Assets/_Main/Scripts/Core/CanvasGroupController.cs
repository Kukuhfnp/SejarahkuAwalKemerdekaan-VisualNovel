...
public class CanvasGroupController
{
    ...
    public Coroutine Show(float speed = 1f, bool immediate = false)
    {
        ...
        {
            DialogueSystems.instance.StopCoroutine(co_hiding);
            co_hiding =null;
        }
        co_showing = DialogueSystems.instance. StartCoroutine(Fading(1, speed, immediate));
    ...
    public Coroutine Hide(float speed = 1f, bool immediate = false)
    {
        ...
        {
            DialogueSystems.instance.StopCoroutine(co_showing);
            co_showing =null;
        }
        co_hiding = DialogueSystems.instance.StartCoroutine(Fading(0, speed, immediate));

        return co_hiding;
    }

    private IEnumerator Fading(float alpha, float speed = 1f, bool immediate = false)
    {
